...
for i in xrange(20):
    for j in xrange(20):
        if i+3 < 20: res = max(res, grid[i][j] * grid[i+1][j] * grid[i+2][j] * grid[i+3][j])
        if j+3 < 20: res = max(res, grid[i][j] * grid[i][j+1] * grid[i][j+2] * grid[i][j+3])
        if i+3 < 20 and j+3 < 20: res = max(res, grid[i][j] * grid[i+1][j+1] * grid[i+2][j+2] * grid[i+3][j+3])
        if i+3 < 20 and j-3 >= 0: res = max(res, grid[i][j] * grid[i+1][j-1] * grid[i+2][j-2] * grid[i+3][j-3])
